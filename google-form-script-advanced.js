// Google Form送信時に自動メール送信 + スプレッドシート記録するスクリプト（高度版）
...
function onFormSubmit(e) {
  try {
    ...
    for (var i = 0; i < itemResponses.length; i++) {
      ...
      // メールアドレス
      if (!email && (title.indexOf("メール") !== -1 || title.indexOf("Email") !== -1 || title.indexOf("email") !== -1)) {
        email = response;
      }
      // 名前
      if (title.indexOf("お名前") !== -1 || title.indexOf("氏名") !== -1 || title.indexOf("名前") !== -1) {
        name = response;
      }
      // 参加希望日
      ...
      // 電話番号
      if (title.indexOf("電話") !== -1 || title.indexOf("TEL") !== -1 || title.indexOf("Tel") !== -1) {
        phone = response;
      }
      // 会社名
      if (title.indexOf("会社") !== -1 || title.indexOf("組織") !== -1 || title.indexOf("所属") !== -1) {
        company = response;
      }
      // 備考・その他
      if (title.indexOf("備考") !== -1 || title.indexOf("その他") !== -1 || title.indexOf("質問") !== -1) {
        remarks = response;
      }
    }
...
function sendAdminNotification(email, name, eventDate, applicationNumber) {
  ...
  var body = "新しい申込がありました。\n\n" +
             ...
             SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(SHEET_NAME).getSheetId();
...
function generateSummaryByDate() {
  ...
  var data = sheet.getRange(2, 7, lastRow - 1, 4).getValues(); // G列〜J列（参加希望日、参加費、ステータス、リピーター区分）
